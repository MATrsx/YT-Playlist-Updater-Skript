...
def check_dependencies():
    ...
    if not shutil.which('node'):
        print("⚠️  WARNUNG: Node.js nicht gefunden!")
        print("   Installiere es mit: sudo apt-get install nodejs")
        print("   Node.js wird für YouTube JS-Challenge-Lösung benötigt")
    else:
        print("✓ Node.js gefunden")
...
def get_playlist_videos():
    ...
    ydl_opts = {
        **YTDLP_BASE_OPTS,
        ...
        'no_warnings': False,
        'ignoreerrors': True,  # Nicht verfügbare Videos überspringen
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if 'entries' in info:
                videos = [(entry['id'], entry['title']) for entry in info['entries'] if entry and entry.get('id')]
                print(f"  ✓ {len(videos)} verfügbare Videos gefunden")
                return videos
